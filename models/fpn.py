...
import torch # Added torch import for torch.norm in the final SparseTensor creation


class BasicBlockBase(nn.Module):
# ... (BasicBlockBase remains the same) ...
    expansion = 1
...
# ... (BottleneckBase remains the same) ...
    expansion = 4
    NORM_TYPE = NormType.BATCH_NORM
...
class PredictionHead(ME.MinkowskiNetwork):
    """MLP Head for the Student Network in the Teacher-Student framework."""
    def __init__(self, in_channels, out_channels, D=3):
        super(PredictionHead, self).__init__(D)
        
        # A simple MLP head on the final feature space
        self.fc1 = ME.MinkowskiLinear(in_channels, in_channels, bias=True)
        self.relu = ME.MinkowskiReLU(inplace=True)
        self.fc2 = ME.MinkowskiLinear(in_channels, out_channels, bias=True)

    def forward(self, x):
        # x is an ME.SparseTensor (interpolated feature)
        out = self.fc1(x)
        out = self.relu(out)
        out = self.fc2(out)
        return out
...
class Res16FPNBase(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, D):
        # ... (network_initialization logic remains the same) ...
        # Setup net_metadata
        dilations = self.DILATIONS
        bn_momentum = 0.02
    ...
    def forward(self, x):
        ...
        out = self.bn4(out)
        out = self.relu(out)
        out_final_backbone = self.block4(out)###256

        out = self.delayer1(out_final_backbone)
        
        # Create TensorField with matching dtype from the SparseTensor
        # Use the dtype from 'out' (the SparseTensor we're interpolating from)
        x = ME.TensorField(
            x.F,
            x.C.to(dtype=out.dtype, device=x.F.device),
        ...
        # Final interpolated features (SparseTensor)
        features_sum = out.F + dout_b3p8.F + dout_b2p4.F + dout_b1p2.F

        # Robustly find a coordinate_map_key / coordinate_manager from any available SparseTensor
        coord_map_key = None
        coord_manager = None
        for t in (out, dout_b3p8, dout_b2p4, dout_b1p2, out_final_backbone, y, x):
            if t is None:
                continue
            if coord_map_key is None and hasattr(t, "coordinate_map_key"):
                coord_map_key = getattr(t, "coordinate_map_key")
            if coord_manager is None and hasattr(t, "coordinate_manager"):
                coord_manager = getattr(t, "coordinate_manager")
            if coord_map_key is not None and coord_manager is not None:
                break

        # As a last attempt, try the sparse view 'y'
        if (coord_map_key is None or coord_manager is None) and hasattr(y, "coordinate_map_key"):
            coord_map_key = getattr(y, "coordinate_map_key", coord_map_key)
            coord_manager = getattr(y, "coordinate_manager", coord_manager)

        if coord_map_key is None or coord_manager is None:
            raise RuntimeError(
                "Cannot find coordinate_map_key or coordinate_manager for SparseTensor creation. "
                "Check MinkowskiEngine version and the tensors: out, dout_b3p8, dout_b2p4, dout_b1p2, out_final_backbone, y, x"
            )

        # Normalize features and construct SparseTensor using normalized features.
        final_feats_F = features_sum
        final_feats_F = final_feats_F / (torch.norm(final_feats_F, p=2, dim=1, keepdim=True) + 1e-12)

        # Construct a new SparseTensor with the normalized features (do not mutate .F)
        final_feats_sparse = ME.SparseTensor(
            features=final_feats_F,
            coordinate_map_key=coord_map_key,
            coordinate_manager=coord_manager,
        )

        # Return the SparseTensor (for Student head) and the normalized dense features (for clustering/teacher target)
        return final_feats_sparse, final_feats_F
