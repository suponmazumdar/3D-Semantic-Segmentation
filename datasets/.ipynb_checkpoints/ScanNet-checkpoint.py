...
class Scannetval(Dataset):
    ...
    def __getitem__(self, index):
        #added by Rahul
        # import open3d as o3d
        # data = o3d.io.read_point_cloud(self.file[index])
        # pcd_down = data.voxel_down_sample(voxel_size=0.05)
        # plane_model, inliers = pcd_down.segment_plane(distance_threshold=0.09,
        #                                       ransac_n=3,
        #                                       num_iterations=1000)

        # # Extract the floor points
        # floor_points = pcd_down.select_by_index(inliers)
