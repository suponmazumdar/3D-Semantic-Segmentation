...
def eval_once(args, model, test_loader, classifier, use_sp=False):
    ...
    for data in test_loader:
        with torch.no_grad():
            ...
            out_sparse, feats = model(in_field)  
            ...
            numpy_pred = preds.numpy()
            # this is for saving predictions
            
            # np.save('pred.npy',numpy_pred)
            # print(len(numpy_pred))
...
def eval(epoch, args):
    ...
    '''Hungarian Matching'''
    row_ind, col_ind = linear_assignment(histogram.max() - histogram)
    o_Acc = histogram[row_ind, col_ind].sum() / histogram.sum() * 100.
    m_Acc = np.mean(histogram[row_ind, col_ind] / histogram.sum(1)) * 100
    hist_new = np.zeros((sem_num, sem_num))
    for idx, col in enumerate(col_ind):
        hist_new[:, idx] = histogram[:, col]
