...
def parse_args():
    ...
    parser.add_argument('--save_path', type=str, default='ckpt/ScanNet/',
                        help='model savepath')
    parser.add_argument('--max_epoch', type=list, default=[350, 600], help='max epoch')
...
def main(args, logger):
    ...
    # for epoch in range(1, 20): #added by Rahul
        ...
        if epoch % 10 == 0:
            ...
            # torch.save(optimizer.state_dict(), join(args.save_path, 'opt_' + str(epoch) + '_checkpoint.pth'))
            with torch.no_grad():
    ...
    for epoch in range(1, args.max_epoch[1] + 1):
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), join(args.save_path,  'model_' + str(epoch) + '_checkpoint.pth'))
            torch.save(classifier.state_dict(), join(args.save_path, 'cls_' + str(epoch) + '_checkpoint.pth'))
            # torch.save(optimizer.state_dict(), args.save_path + 'opt_' + str(epoch) + '_checkpoint.pth')
            with torch.no_grad():
                o_Acc, m_Acc, s = eval(epoch, args)
                logger.info('Epoch: {:02d}, oAcc {:.2f}  mAcc {:.2f} IoUs'.format(epoch, o_Acc, m_Acc) + s)
...
def cluster(args, logger, cluster_loader, model, epoch, start_grow_epoch=None, is_Growing=False):
    ...
    primitive_labels = KMeans(n_clusters=args.primitive_num, n_jobs=-1).fit_predict(sp_feats.numpy().astype(np.float32))
...
def train(train_loader, logger, model, optimizer, loss, epoch, scheduler, classifier):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        loss_sem = loss(logits * 5, pseudo_labels_comp).mean()## 5 is temperature
